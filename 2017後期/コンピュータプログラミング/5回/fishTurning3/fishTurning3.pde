...
void draw() {
  ...
  if (mouseX>x+2 && x<width-30) {
    Time=Time+0.1;
  } else if (mouseX<x-2 && x>30) {
    Time=Time-0.1;
  } 
  x = Time * 50 + 100;
  fish1.drawFish(x, fish,100);
  fish2.drawFish(x, fish,200);
  fish3.drawFish(x, fish,300);
}
void mousePressed() {
...
class Fish {
  void drawFish(float x, int[]fish,int y) {
    // 魚の姿の描画
    //y =100;
    println("(" + x + "," + y + ")");
