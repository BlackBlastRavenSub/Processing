...
void draw() {
  ...
  i=1;
  text("Left"+(MAXBALLNUMBER-currentBallNumber),10,50);
}
...
void keyPressed() {
  if (key == ' '&&oneshot) {
    if (MAXBALLNUMBER>currentBallNumber) {
      currentBallNumber++;
      //d=d*2/3;
      ball[currentBallNumber-1]=new Ball(100, 100, 5, 5);
      //ball[currentBallNumber-1]=new Ball(this.X, this.Y, this.VX, -5, this.d, this.Hue);
      //100, 100, 5, 5,this.Hue
    }
    System.out.println("増えた");
    oneshot=false;
  }
  if(key == ENTER){
  if (MAXBALLNUMBER>currentBallNumber) {
