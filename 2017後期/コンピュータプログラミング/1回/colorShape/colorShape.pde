...
void draw() {
  ...
  float Ellipse_X=Center_X+cos(Radian)*Circle_Radius*Round;//円の中心のX座標//円のX座標指定(キャスト)
  float Ellipse_Y=Center_X+sin(Radian)*Circle_Radius*Round;//円の中心のY座標//円のY座標指定(キャスト)
  ellipse(Ellipse_X, Ellipse_Y, 10, 10);
  Pos=(Pos+1)%Circle_N;//Posの数取得
  Hue=(Hue+1000/Circle_N*0.5)%360;//色関数指定
  print("TEST");
  
  float deg2=360*Pos/Circle_N;//360度/N=円一つあたりの角度→*今の円の数
  float Radian2=radians(deg2);
  Round=sin(Radian2*i);
  i=(i+0.1)%10;
}