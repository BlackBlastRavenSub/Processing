...
void setup() {
  ...
  turtle =new Turtle[3];
  turtle[0] = new Turtle(turtleD, width / 7, height - turtleD);
  turtle[1] = new Turtle(turtleD, width / 2, height - turtleD);
  turtle[2] = new Turtle(turtleD, width / 1.2, height - turtleD);
}
...
class Turtle {
  ...
  void move() {
    ...
    //接触判定
    if (shell.touch()||head.touch()||foot[0].touch()||foot[1].touch()||foot[2].touch()||foot[3].touch()) {
      // 移動
      x += vx;
      y += vy;
